...
/// the `FULLSCREEN` flag. In `LayoutKind::Grid` (no scroll ribbon to join)
/// and in Overview (the fullscreen tile is shown scaled, like any other tile)
/// `col` is `None` and the window falls back to its normal tile slot.
#[derive(Debug, Clone, Copy, Default)]
pub struct FsCtx {
    /// Index of the column hosting the fullscreen window, if any.
    pub col: Option<usize>,
    /// The fullscreen window of that column.
    pub win: Option<WindowId>,
    /// The full-screen box (`mon.screen`) the tile should fill.
    pub screen: Rect,
}

/// Pure derivation of `FsCtx`. Returns `col: None` when the workspace is not a
/// `Column` layout or is in Overview (the fullscreen tile is then just a
/// normal, scaled, ribbon participant and never an overlay).
///
...
fn arrange_columns(state: &State, mon: &Monitor, cfg: &Cfg, out: &mut Placements) {
    ...
    // Single source of truth: the live (un-settled) ribbon geometry.
    let g = ribbon_geom(ws, cfg, full_wa, false, fs);
    let wa = g.wa;
    ...
    for (col_idx, col) in ws.columns.iter().enumerate() {
        ...
        if Some(col_idx) == fs.col {
            let (world_x, _col_w_world) = g.cols[col_idx];
            if let Some(win) = fs.win {
                let screen = fs.screen;
                let alpha = g.alpha;
                let cx = g.cx;
                let screen_col_x =
                    (wa.x as f32 + (world_x - ws.camera.position) * alpha + cx).round() as i32;
                ...
                if state.clients.contains_key(&win) {
                    out.push((win, Rect::new(screen_col_x, screen_y, screen_w, screen_h), 0));
                }
            }
            continue;
        }
        ...
        let row_info: Vec<(f32, f32)> = if n == 1 {
            vec![(total_h.max(1.0), wa.y as f32)]
        } else {
            (0..n)
                .map(|i| {
                    let extra = if i == n - 1 {
                    ...
                    let h = (base_h + extra).max(1.0);
                    let y = wa.y as f32 + i as f32 * (base_h + gap_f);
                    (h, y)
                })
                .collect()
        };
        ...
        for (ri, &win) in col.windows.iter().enumerate() {
            ...
            let (row_h_world, row_y_world) = row_info[ri];
            let screen_h = (row_h_world * alpha).max(1.0) as u32;
            let screen_y =
                (wa.y as f32 + (row_y_world - wa.y as f32) * alpha + cy).round() as i32;
...
pub fn ideal_scroll(ws: &Workspace, cfg: &Cfg, workarea: Rect, fs: FsCtx) -> f32 {
    ...
    if Some(i) == fs.col && ws.layout == LayoutKind::Column {
...
mod tests {
    ...
    fn place(state: &mut State, cfg: &Cfg) -> Placements {
        let fs = fs_ctx(&state.clients, state.monitors[0].ws(), state.monitors[0].screen);
        ...
        arrange_columns(state, &state.monitors[0], cfg, &mut out);
        out
    }
    ...
    fn fullscreen_column_scrolls_away() {
        ...
        let scroll0 = ideal_scroll(state.monitors[0].ws(), &cfg, state.monitors[0].workarea, fs0);
        ...
        let scroll1 = ideal_scroll(state.monitors[0].ws(), &cfg, state.monitors[0].workarea, fs1);
        state.monitors[0].workspaces[0].camera.position = scroll1;
        let mut out2 = Placements::new();
        arrange_columns(&state, &state.monitors[0], &cfg, &mut out2);
    ...
    fn ribbon_invariants_hold_with_fullscreen() {
        ...
        let extents = column_screen_extents(state.monitors[0].ws(), &cfg, state.monitors[0].workarea, fs);
